...
const enhancedReadings = [
  {
    title: 'Online Grocery Shopping Basics',
    summary: 'Learn how to order groceries online safely and efficiently',
...
Online grocery shopping saves time and can help you stick to a budget by avoiding impulse purchases. With practice, you'll become efficient at finding what you need and ensuring you get quality products delivered to your door.`,
    ...
    topic_tags: 'shopping,groceries,basics',
  ...
  {
    title: 'Understanding Digital Communication',
    summary: 'Navigate modern ways people communicate online and through text',
...
Understanding these communication styles helps you connect better with people of all ages and feel more confident participating in digital conversations.`,
    difficulty_level: 'easy',
    topic_tags: 'communication,digital,social',
    discussion_questions: [
...
async function seedEnhancedReadings() {
  const db = new sqlite3.Database(dbPath);

  try {
    console.log('🔄 Updating readings with enhanced content...');

    for (const reading of enhancedReadings) {
      await new Promise((resolve, reject) => {
        db.run(
          ...
          [
            reading.content,
            JSON.stringify(reading.discussion_questions),
            reading.title
          ],
          function(err) {
            if (err) {
              console.error(`Error updating ${reading.title}:`, err);
              reject(err);
            } else {
              console.log(`✅ Updated: ${reading.title}`);
              resolve();
            }
          }
        );
    ...
    console.log('✅ Enhanced readings seeded successfully!');
  } catch (error) {
    console.error('❌ Error seeding enhanced readings:', error);
    throw error;
  } finally {
    db.close();
  }
...
module.exports = { seedEnhancedReadings };
