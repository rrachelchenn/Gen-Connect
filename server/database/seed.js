...
async function seedDemoUsers() {
  ...
  try {
    ...
    // Additional demo tutors
    db.run(`INSERT OR IGNORE INTO users (email, password, name, role, college, major, bio) 
            VALUES (?, ?, ?, ?, ?, ?, ?)`, 
           ['sarah.tutor@genconnect.com', hashedPassword, 'Sarah Miller', 'tutor', 'UC Berkeley', 'Information Technology', 
            'I\'m Sarah, an IT student who enjoys teaching technology in simple, easy-to-understand ways. I specialize in smartphone basics and social media safety.']);

    db.run(`INSERT OR IGNORE INTO users (email, password, name, role, college, major, bio) 
            VALUES (?, ?, ?, ?, ?, ?, ?)`, 
           ['mike.tutor@genconnect.com', hashedPassword, 'Mike Rodriguez', 'tutor', 'MIT', 'Digital Media', 
            'Hello! I\'m Mike, and I love helping people discover the creative possibilities of technology. From photography to video calls, I make tech fun and approachable.']);
